...
- Extraction = encoding each face (eyes, nose, jawline) so you can recognise the same person later.
...
From there you can compute per-blob stats like area, bounding box, centroid, etc.
...
Feature extraction is the bridge between **raw pixels** and **high-level understanding**. Good descriptors preserve the information that matters for your task while throwing away everything else.
